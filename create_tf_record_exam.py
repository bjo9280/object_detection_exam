...
def dict_to_tf_example(data,
  ...
  """
  img_path = os.path.join(image_subdirectory, data['path']) #filename > path
  #image_format = str.encode(data['path'].split('.')[1])
  with tf.gfile.GFile(img_path, 'rb') as fid:
    encoded_jpg = fid.read()
  encoded_jpg_io = io.BytesIO(encoded_jpg)
  image = PIL.Image.open(encoded_jpg_io)
  img = np.array(image)
  print("{} {} {}".format(img_path, image.format ,img.shape))
  #if image.format != 'JPEG':
  # raise ValueError('Image format not JPEG')
  key = hashlib.sha256(encoded_jpg).hexdigest()

  width = int(data['size']['width'])
  assert width > 0, (width, data['path']) #filename > path
  height = int(data['size']['height'])
  assert height > 0, (height, data['path']) #filename > path
...
def main(_):
  ...
  train_output_path = os.path.join(FLAGS.output_dir, 'datasetname_train.record')
  val_output_path = os.path.join(FLAGS.output_dir, 'datasetname_val.record')
